...
export class EnregistrementComponent implements OnInit {
  ...
  onDelete(item: StockOT): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.stockageService.deleteStockOT(item.codeComplet!).subscribe({
          next: () => {
            this.loadAll();
          },
          error: (err) => {
  ...
  onModify(item: StockOT): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.stockageService.updateStockOT(result).subscribe({
          next: () => {
            this.loadAll();
          },
          error: (err) => {
